...
```
#### Git commit. Inserta los archivos que se encuentren en el staging área y los coloca en el repositorio. Hay diferentes formas de usar el "git commit". Ejemplo:
```Shell 
git commit -m "nombre del título de commit". 

git commit --amend. Corrige el commit mas reciente.

El "git add" junto al "git commit" se define como el proceso básico de GIT
```
...
```Shell
Para usarlo entre commits es: git checkout [ID del commit]

Para usarlo entre ramas es: git checkout [nombre de la rama]
```
#### Git branch. Te permite crear una rama nueva. Su iteración es: 
```Shell 
git branch [nombre de la rama a crear]. Para crearla.

gti checkout [nombre de la rama creada]. Para moverte hacia ella. 

"Una vez creada, debes ubicarte en ella para comenzar a trabajar en tu nueva rama"
```
#### Git log. Te ayuda a explorar las previas revisiones de un proyecto: 
```Shell 
git log
```
#### Git alias. Es un comando que te permite crear tus propios comnados de GIT a partir de otros:
```Shell
...
Este comando te permite ver "git log" de una manera más dinámico, aparte de que ayuda mucho a la hora de usar "git log"
```
...
```
#### Git push. Te permite mover una rama local a otro repositorio, usualmente es la forma de publocar las contribuciones en un servidor remoto.
```Shell
git push [nombre del remoto] [nombre de la rama]
```
#### Git remote. Te permite conectar repositorios locales y remotos. Sirve para que en lugar de poner la URL para realizar "fetch o pull", solo le asignaremos un nombre y así podremos llamar a esa conexión de manera rápida.
```Shell 
git remote add [nombre del remoto]

Por defecto se llama "origin".
```
